...
try {
    ...
    $response = $sdk
        ->api()
        ->tasks()
        ->notionAdd($arg_team_id, $arg_channel_id, $arg_item_id, $arg_task_id, $arg_notion_id, $arg_x_org_id);

    var_export($response);
} catch (\Kronup\Sdk\ApiException $apiExc) {
    echo sprintf(
        "API Exception when calling api()->tasks()->notionAdd(): %s\n", 
        var_export($apiExc->getResponseObject(), true)
    );
} catch (\Exception $exc) {
    echo sprintf(
        "Exception when calling api()->tasks()->notionAdd(): %s\n", 
        $exc->getMessage()
    );
}